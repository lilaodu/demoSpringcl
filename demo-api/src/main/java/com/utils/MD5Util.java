...
public class MD5Util {
    ...
    public  static String generateSign(Map<String,Object> resultMap,String client_secret_pd){
        ...
        try {
            ...
            sb.append(client_secret_pd);//首加上secret
            for (int i = 0; i <paramArr.length ; i++) {
                String key = paramArr[i];
                if(key.equals("sign"))
                    continue;
                String value = resultMap.get(key).toString();
                sb.append(key).append(value);
            }
            sb.append(client_secret_pd);//尾加上secret
            String lastStr = sb.toString();
            System.out.println(lastStr);
            String digest = getMD5(lastStr);
